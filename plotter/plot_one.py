        ...
        with open(e['f_tput']) as f:
            ...
            for row in reader:
                ...
                if elapsed==20 or elapsed == 30 or elapsed == 40:
                    avg += int(successful)
        #e['avg_tput']= ((avg/3)/10)*((rounds*reads)+writes) # MultiDC MultiRound
        e['avg_tput']= ((avg/3)/10)*(1022+writes) # For exponential
...
plt.xlabel('Throughputput [ops/sec]')
